...
接下来通过修改 *[Hello world](#hello-world)* 的示例，得到三个原子类型 $HelloAtom$ 的实例。我们强制使它们的变迁同步（即*交互*，*rendez-vous*）:
...
由于接收器中由 $p$ 标记的变迁存在守卫条件的约束，初始变迁执行之后，使能的交互如下：$s$、$s,r1$、 $s,r3$ 和 $s,r1,r3$ （$r2$ 传入的值为2，不满足约束）。又因为给定一个链接器，BIP 默认给予有更大范围的交互更高的优先级，最终选取了最大交互
$s,r1,r3$  执行。
...
BIP2 语言的大多数用例都考虑使用原子组件中的自动机进行表示。在 BIP2中，也可以使用 *1-safe* Petri 网。
...
> 译注：*内部*（*internal*）变迁对其他组件不可见，并优先于其他可被（其他组件）观察到的变迁。其执行与否取决于当前（组件的）状态和所关联的守卫条件。
